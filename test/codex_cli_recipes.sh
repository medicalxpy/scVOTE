...
# （2b）对 results/ 下已有的所有单次训练结果批量跑 evaluation.py，并汇总成一个 CSV
# - 每个 run 的 stdout/stderr 会写到 logs/eval_<run_name>.log
# - CSV 会写到 results/evaluation/all_runs_metrics.csv
# nohup bash -c 'cd /data1021/xiepengyu/scVOTE && bash test/eval_all_results.sh results results/evaluation/all_runs_metrics.csv' \
#   > logs/eval_all_results_driver.log 2>&1 &
